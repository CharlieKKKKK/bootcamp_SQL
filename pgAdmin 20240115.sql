...
CREATE TYPE report_data_type AS (
    borrow_month VARCHAR,
    user_id INT,
    num_of_book INT
);

CREATE OR REPLACE FUNCTION insert_management_report(borrow_month text) RETURNS VOID AS $$
DECLARE
	rec report_data_type;
BEGIN
	FOR rec IN 
        ...
        FROM BORROW_HISTORY BH
        WHERE TO_CHAR(BH.BORROW_DATE, 'YYYY') = borrow_month
