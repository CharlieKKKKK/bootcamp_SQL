...
values (3, '12345678', 'a@gmail.com');
insert into user_contact (user_id, user_phone, user_email)
values (4, '87654321', 'b@gmail.com');

insert into book (author_id, book_name) values (4, 'ABC BOOK');
insert into book (author_id, book_name) values (3, 'DEF BOOK');
insert into book (author_id, book_name) values (3, 'IJK BOOK');
...
return trigger as $$
begin
   -- new.borrow_times := old.borrow_times +1;
   update users set borrow_times = borrow_times_trigger +1 where id = new.user_id
   return new;
end;
$$ language plpgsql;

create or replace trigger borrow_times_trigger
after insert -- or before
on borrow_history
for each row
execute function borrow_times_increment();

-- Trigger event (insert into borrow_history)
insert into borrow_history (user_id, borrow_date)
values (3,1 '2023-01-15 13:30:00');
